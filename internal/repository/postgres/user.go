...
func (r *UserRepo) CreateWithRole(ctx context.Context, email, passwordHash, roleName string) (*domain.User, error) {
	...
	var u domain.User
	if err := tx.QueryRow(ctx, insertUser, email, passwordHash).Scan(
	...
	if _, err := tx.Exec(ctx, insertRole, u.ID, roleName); err != nil {
		return nil, err
	}
